...
export type TestQuery = { __typename?: 'Query' } & {
  assetCollection?: Maybe<
    { __typename?: 'AssetCollection' } & {
      items: Array<Maybe<{ __typename?: 'Asset' } & AssetFragment>>
    }
  >
}
...
export const TestDocument = gql`
  query Test($preview: Boolean!) {
    assetCollection(limit: 1, preview: $preview) {
      items {
        ...Asset
      }
    }
...
export function getSdk(
  ...
  return {
    Test(variables: TestQueryVariables): Promise<TestQuery> {
      return withWrapper(() =>
        client.request<TestQuery>(print(TestDocument), variables)
      )
    }
  }
}
